...
如果你要同时启动后端 API 和前端开发服务器，建议打开两个终端：一个用于后端，一个用于前端。

### 后端（Backend）

在 `c:\Projects\agent` 目录下运行：
...
- 这会启动后端服务，供前端 `agent-web` 调用。
- 如果依赖已经安装过，可以省略 `pip install -r requirements.txt`。

### 前端（Frontend）

在 `c:\Projects\agent-web` 目录下运行：
...
- `npm install` 只需第一次运行或依赖变更后执行。
- `npm run dev` 会启动 Vite 开发服务器，并显示本地访问地址。

---

## 七、选解释器（运行/调试时用哪个 Python）
...
新增脚本：
run_daily_ecommerce_update.py
install_daily_ecommerce_update_task.ps1
每天 11:30 会自动跑任务：
Agent Daily Ecommerce Update
默认跑“昨天”的数据，流程是：
同步 EZLIFE 乐天订单
同步 tomtoc 乐天订单
更新 tomtoc 厂家周维度数据
...
更新 tomtoc 仪表盘
日志会保存在：
C:\Projects\agent\logs\daily_ecommerce_update_YYYYMMDD_HHMMSS.json